...
export const run
	= async <Tokens extends Array<interfaces.ServiceIdentifier<unknown>>>(def: EntryPoint<Tokens>): Promise<void> => {
		const c = await generateContainer();
		const deps = await Promise.all(
			def.dependencies.map(async (d) => c.get(d)),
		) as interfaces.ArgsForInjectionIdentifiers<Tokens>;
